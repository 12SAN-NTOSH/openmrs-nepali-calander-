...
function dateToInternationalizedDate(
  ...
  if (date instanceof CalendarDate || date instanceof CalendarDateTime || date instanceof ZonedDateTime) {
    ...
    const calendarDate = new CalendarDate(nepaliDate.getYear(), nepaliDate.getMonth() + 1, nepaliDate.getDate());
...
function internationalizedDateToDate(date: DateValue): Date | undefined {
  ...
  const isoDate = date.toDate(getLocalTimeZone());
  const nepaliDate = new NepaliDate(isoDate);
  return nepaliDate;
}
...
const MonthYear = forwardRef<HTMLSpanElement, PropsWithChildren<HTMLAttributes<HTMLSpanElement>>>(
  function MonthYear(props, ref) {
    ...
    const nepaliDate = new NepaliDate(state.visibleRange.start.toDate(tz));
    const month = nepaliDate.format('MMMM');
            ...
            >
              ...
              <Popover className={styles.popover} placement="bottom" offset={1}>
                <Dialog className={styles.dialog}>
                  ...
                  {(date) => {
                    const nepaliDate = new NepaliDate(date.toDate(getLocalTimeZone()));
                    return (
                      <CalendarCell
